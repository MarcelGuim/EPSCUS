...
from dispositiu import find_if_there_is_emergency_lane, find_if_street_has_traffic_light, get_light_state, change_traffic_light_for_one_lane, change_traffic_lights_for_whole_edge, get_light_state_for_edge, create_emergency_lane, find_if_street_has_traffic_light_for_edge, activate_emergency_lane_in_edge, change_lights_for_case_only_lights
...
def run_simulation_with_vehicles_and_traffic_lights_activated_for_given_traffic(edges, vehicles_added, net):
    total_time = None
    result = None
    set_speed = []
    ...
    for e in edges:
        ...
        if tls != None:
            if edges.index(e) != 0:
                previous_edge = edges[edges.index(e) - 1]
            else:
                possible_origin = net.getEdge(current_edge).getIncoming()
                options = []
                for p in possible_origin:
                    options.append(p.getID())
                previous_edge = random.choice(options)
            current_next_light, next_state, next_idx = get_light_state_for_edge(e, tls)  
            lights_to_change.append(change_lights_for_case_only_lights(tls, "G", next_state, next_idx, net, edges, previous_edge, e))
...
def run_simulation_with_vehicles_andtraffic_light_activation_random_traffic(edges, initial_amount, simulation_amount,):
    total_time = None
    result = None
    ...
    while not stop:
        ...
        if veh_id in traci.vehicle.getIDList(): 
            edge = traci.vehicle.getRoadID(veh_id)
            allowed_for_this_edge = traci.lane.getAllowed(e+"_0")
            lane_number = traci.vehicle.getLaneIndex(veh_id)
            if len(allowed_for_this_edge) == 1 and "emergency" in allowed_for_this_edge and lane_number != 0:
                traci.vehicle.changeLane(veh_id, 0, 100)
            x,y = traci.vehicle.getPosition(veh_id)
