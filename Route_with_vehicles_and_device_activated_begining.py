...
def run_simulation_with_vehicles_and_device_and_given_traffic(edges, vehicles_added):
    total_time = None
    result = None
    ...
    for e in edges_without_emergency_path:
        ...
        if tls:
            ...
            tls_s.append(tls)
            for e_o in outgoing:
                tls = find_if_street_has_traffic_light_for_edge(e_o.getID())
                if tls:
                    tls_s.append(tls)
                    current_next_light, next_state, next_idx = get_light_state_for_edge(e_o.getID(), tls)        
                    change_traffic_lights_for_whole_edge(tls, "G", next_state, next_idx, previous_edge, e_o.getID())
            try:
                next_next_edge = get_next_valid_edge(e, edges)
                if next_next_edge is not None:
                    lights_to_change.append([next_next_edge, tls_s])
...
def run_simulation_with_vehicles_and_device_random_traffic(edges, initial_amount, simulation_amount,):
    total_time = None
    result = None
    edges_with_no_tls_and_one_lane = None
    edges_with_tls_and_one_lane = None
